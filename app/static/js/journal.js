    ...
    for (let i = 0; i < filteredPoints.length - 1; i++) {
        ...
        segment.on('mouseover', function (e) {
            ...
            L.popup({ closeButton: false, className: 'dist-tooltip', offset: [0, -10] })
                .setLatLng(e.latlng)
                .setContent(
                    `<div class="text-[10px] font-mono p-1 text-white text-center">${dist.km} km<br>${dist.miles} mi</div>`
                )
                .openOn(map);
        });
...
fetch('/api/points')
    ...
    .then((points) => {
        ...
        uniqueDates.forEach((date, index) => {
            ...
            btn.className =
                'day-btn px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest';
