        ...
        .then(points => {
            ...
            sortedDates.forEach(date => {
                ...
                groups[date].forEach(p => {
                    ...
                    item.className = 'gallery-item aspect-square cursor-pointer border border-slate-800 rounded-sm';
...
window.openLightbox = function(photoData, entryNumber) {
    ...
    document.getElementById('lb-alt').innerText = photoData.alt ? `${Math.round(photoData.alt)}m` : '---';
    document.getElementById('lb-time').innerText = photoData.time || 'Unknown';
    document.getElementById('lb-coords').innerText = `${photoData.lat.toFixed(4)}, ${photoData.lon.toFixed(4)}`;
